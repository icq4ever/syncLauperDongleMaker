...
type License struct {
	...
	USBSerial string `json:"usb_serial,omitempty"` // SHORT 우선, 없으면 LONG 폴백 값 저장
	IssuedAt  int64  `json:"issued_at"`

	// 선택(기본 0=미사용). 플래그는 아직 붙이지 않음.
...
func getUSBSerials(dev string) (shortID, longID string) {
	...
	for _, ln := range strings.Split(out, "\n") {
		...
		} else if strings.HasPrefix(ln, "ID_SERIAL=") {
...
func readSerial(dev string) (serialShort, serial string) {
	// 유지: 기존 호출 경로 호환용
	return getUSBSerials(dev)
}

func gatherIDs(mount string) (usbSerial string, fsUUID string, partUUID string, err error) {
	part, parent, err := sourceDevice(mount)
	if err != nil {
		return "", "", "", err
	}
	ss, s := readSerial(parent)
	usbSerial = pickUSB(ss, s) // SHORT 우선
...
func verify(bytes, sig []byte, pub ed25519.PublicKey) bool {
...
func cmdIssue(mount, privPath, licensee, keyID string) error {
	// mount로부터 parent를 얻어 SHORT 우선 시리얼을 계산
	part, parent, err := sourceDevice(mount)
	if err != nil {
		return err
	}
	ss, s := getUSBSerials(parent)
	...
	if err := writeIfChanged(filepath.Join(mount, "license.json"), jb, 0644); err != nil {
		return err
	}
	if err := writeIfChanged(filepath.Join(mount, "license.sig"), []byte(base64.StdEncoding.EncodeToString(sig)), 0644); err != nil {
		return err
	}
	fmt.Printf("issued to %s\n  fs_uuid=%q\n  partuuid=%q\n  usb_serial=%q\n", mount, strings.TrimSpace(fsUUID), strings.TrimSpace(partUUID), usbSerial)
	return nil
}
...
func bakeISO(opts BakeOpts) error {
	...
	lic := &License{
		...
		USBSerial: strings.TrimSpace(usbSerial), // SHORT or LONG fallback
		IssuedAt:  time.Now().Unix(),
	}
...
func bakeFAT32(opts BakeOpts) error {
	...
	part, err := createMBRSinglePartition(opts.Target, "fat32")
	if err != nil {
		return err
	}
...
func bakeEXT4(opts BakeOpts) error {
	...
	part, err := createMBRSinglePartition(opts.Target, "ext4")
	if err != nil {
		return err
	}
...
func listCandidateDisks() ([]DevInfo, error) {
	...
	for _, ln := range strings.Split(out, "\n") {
		...
		// e.g. "sdb 14.6G SanDisk Ultra    usb disk"
...
func interactiveBake(base BakeOpts) (BakeOpts, error) {
	...
	if opts.Mode == "" {
		opts.Mode = ModeFAT32 // 기본안내: fat32
	}
...
func main() {
	if len(os.Args) < 2 {
		...
		fmt.Println("  syncLauperDongleMaker issue   --mount /media/dongle --priv privkey.pem [--licensee NAME] [--key-id K]")
		fmt.Println("  syncLauperDongleMaker verify  --mount /media/dongle --pub  pubkey.pem")
		fmt.Println("  syncLauperDongleMaker bake    [--mode iso|fat32|ext4] [--target /dev/sdX] [--priv privkey.pem] [--licensee NAME] [--key-id K] [--label LABEL] [--readme README.pdf] [--force]")
		fmt.Println("       (no options → interactive wizard)")
		os.Exit(2)
	}
	...
	case "issue":
		...
		if err := cmdIssue(*mount, *priv, *licensee, *keyID); err != nil {
	...
	case "bake":
		...
		// 옵션이 거의 비어있으면 대화형 진입
		if *mode == "" && *target == "" && !*force && *licensee == "" && *label == DefaultLabel && *keyID == "k1" && *readme == "" {
			var err error
